...
from RTGENEModels import RTGENEModelMobileNetV2
...
_transform = transforms.Compose([transforms.Resize((224, 224), Image.BICUBIC),
                                 transforms.ToTensor(),
                                 transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])

device_id_gazeestimation = "cuda:0"
_model = RTGENEModelMobileNetV2()
_torch_load = torch.load(os.path.join(__script_path, "../model_nets/rt_gene_pytorch_checkpoints/_ckpt_epoch_3.ckpt"))['state_dict']
    ...
    # for file in _img_list:
        ...
        if ret:
            ...
            if len(faceboxes) > 0:
                ...
                _left = Image.fromarray(subject.left_eye_color.astype('uint8'), 'RGB')
                _right = Image.fromarray(subject.right_eye_color.astype('uint8'), 'RGB')
                ...
                print("Hz: {}".format(1.0 / (time.time() - stime)))
                ...
                cv2.imshow("patches".format(*gaze[0][::-1]), s_gaze_img)
                cv2.waitKey(1)
