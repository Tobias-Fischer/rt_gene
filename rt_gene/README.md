...
- wget https://github.com/yinguobing/head-pose-estimation/raw/master/assets/frozen_inference_graph.pb -P $(rospack find rt_gene)/model_nets
...
- Calibrate your camera (http://wiki.ros.org/camera_calibration). Then, save the resulting `*.yaml` file to `$(rospack find rt_gene)/webcam_configs/` and change the entry for the `camera_info_url` in the `$(rospack find rt_gene)/launch/start_webcam.launch` file.
