...
1. Install required Python packages:
    - For `conda` users (recommended; create a new environment first if you want): `conda install -c conda-forge dlib tensorflow-gpu numpy scipy tqdm pillow rospkg opencv && conda install -c pytorch pytorch torchvision` (you also might need to `pip install empy`)
